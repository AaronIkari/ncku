...
def main():
    ...
    start_time = time.time()
    slpa.perform_slpa(24)  #perform slpa for 20 iterations
    end_time = time.time()
    print("Elapsed time for slpa was %g seconds" % (end_time - start_time))

    start_time = time.time()
    slpa.post_processing(0.3)  #perform postprocessing with threshhold 0.1
    end_time = time.time()
    ...
    for i in range(len(slpa.node_memory)):
        com = str(sorted(slpa.node_memory[i].items(), key=operator.itemgetter(1), reverse=True)[0][0])
        f_out.write("%d\t%s\n" % (i+1, com))
        num_com.add(sorted(slpa.node_memory[i].items(), key=operator.itemgetter(1), reverse=True)[0][0])
    f_out.close()

    print('# communities %d' %len(num_com))
